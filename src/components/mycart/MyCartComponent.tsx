...
//   // Decide cart source
//   const cartItems =
//     isAuthenticated && userCart?.items?.length ? userCart.items : guestCart;
...
//               {checkAllCart ? `${cartItems.length}/${cartItems.length}` : "0/0"}{" "}
//               Items Selected
//             </Typography>
//           </div>
...
//               onClick={() =>
//                 navigate(
//                   isAuthenticated ? "/user/selectadress" : "/selectadress"
//                 )
//               }
//               // disabled={!isAuthenticated}
...
const normalizeItem = (item: any) =>
  item?.product ? item : { product: item, quantity: item.quantity || 1 };
...
const MyCart = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      ...
      setGuestCart(Array.isArray(localCart) ? localCart : []);
    }
  }, [isAuthenticated]);

  /** ---------- Decide cart source ---------- */
  const cartItems = useMemo(() => {
    return isAuthenticated && userCart?.items?.length
      ? userCart.items
      : guestCart;
  }, [isAuthenticated, userCart, guestCart]);

  /** ---------- Subtotal (NO NaN EVER) ---------- */
  const subtotal = useMemo(() => {
    if (isAuthenticated) {
      return toNumber(userCart?.totalPrice);
    }

    return cartItems.reduce((sum: number, rawItem: any) => {
      const item = normalizeItem(rawItem);
      const price = toNumber(item.product?.discountPrice);
      const qty = toNumber(item.quantity) || 1;
      return sum + price * qty;
    }, 0);
  }, [isAuthenticated, userCart, cartItems]);
              ...
              >
                <CardContent className="flex gap-4 md:gap-10 items-center">
                  ...
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex gap-2">
                      <Typography className="text-[#a56c46] font-bold">
                        ₹ {toNumber(item.product?.discountPrice)}
                      </Typography>
                      <Typography className="line-through text-gray-500">
        ...
        >
          <CardContent>
            ...
            <button
              ...
              className="text-white px-6 py-3 font-semibold w-full bg-primary"
            >
              CHECKOUT
            </button>
